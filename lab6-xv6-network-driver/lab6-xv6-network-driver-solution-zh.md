...
网络这一章相对独立，与前面的 OS lab 风格有点不同：network 部分有许多约定俗成的实现细节，且与内存管理、`pagetable` 关系不大。之前积累的经验和直觉在本章中帮助不大，不过还是值得完成这个实验。

## Part one: NIC

这一部分需要在 `kernel/e1000.c` 中实现两个函数：`e1000_transmit()` 和 `e1000_recv()`。当 `net.c` 调用 `e1000_transmit()` 时，会传入一个已经通过 `kalloc()` 分配好、装有待发送数据的 buffer；而 `e1000_recv()` 则负责把收到的包直接写入内存中的 buffer，用一个接收环形队列（RX ring）管理。

先实现 `e1000_transmit()`。首先需要弄明白两个重点的标识，一个是 `TDT`， 另外一个是 `DD`
...
查看 `e1000_dev.h` 可以看到 `E1000_TDT`，它表示可以向发送环中的这个位置写入数据，这个是最先可用的位置（初始化时 `TDT` 为 0，代表可以直接从索引 0 开始填充——因为一开始环中完全没有数据，标记为 0 即表示 `TDT` 是最先可用的位置）。
...
而`status` 字段中的 `E1000_TXD_STAT_DD`，初始化时会被设为 1，说明该值为 1 时代表这些位置是空闲可用的，可以直接填入数据。0 则代表这个正在被使用。
...
在编写代码之前，先梳理整体思路再动手：拿到数据包后，根据 `TDT` 得到环中的最先可用的位置，检查 `status` 判断该位置是否可写入；若可写入则填入数据包，若 ring 已满则返回错误；填入时记得把 `status` 置为 0，表示该位置已经有数据在处理，还有更新 `TDT`。

提示：还需要设置 `cmd` 字段，这里用到两个标志位：`E1000_TXD_CMD_EOP` 标记"该描述符是这个包的最后一个描述符"，`E1000_TXD_CMD_RS` 告诉网卡"发送完成后请把状态写回描述符（即重新将 DD 位置 1）"。

完整的 `e1000_transmit` 如下：
...
观察初始化过程可以发现，`RDT` 被额外减了 1，因此使用时需要手动加 1 才能得到正确的最先可用的读取位置。hint 中也提到了这一点：
...
同样地，`status` 标记初始化时默认值为 0：`status=0` 表示网卡尚未向该 buffer 写入数据，因此不可读；`dd=1` 表示 buffer 中已写入数据，可以开始读取。

> The e1000 can deliver more than one packet per interrupt

Hint里面提到：由于一次中断可能收到多个包，建议使用 `while` 循环处理这部分逻辑，持续向后索引，直到没有新包为止。

整理一下思路：该函数被调用时，从 `tail+1` 开始处理，这是最后一个 `status` 为 0 的位置（和 `E1000_RXD_STAT_DD` 用位运算确定），说明可以填入新数据；在 `while` 循环中，每取到一个数据包就调用 `net_rx`，处理完成后清空该地址或用 `kalloc` 分配新空间，最后将 `status` 重置为默认值 0，并更新 `tail` 和 `rdt`，在环中向前推进一步。
...
这样 recv 部分就完成了，同样根据 lab 提示，对新实现的功能进行测试：开启两个命令行窗口，一个运行 `make qemu` 进入操作系统，另一个运行 `nettest.py rxone` 向 xv6 发送数据包，收到数据后系统会自动显示接收成功。
...
主要会涉及三个函数：`ip_rx()`、`sys_recv()`、`sys_bind()`。`sys_bind` 用于初始化然后完成端口绑定；`ip_rx` 负责判断收到的包是否为 UDP，以及端口是否匹配已绑定的端口，然后把收到的数据存到队列里面；`sys_recv` 则运行在用户进程侧，从队列中取出 `ip_rx` 存入的数据，并拷贝给用户（拷贝到当前 `proc` 下的内存里面）。

按照惯例，先完成最简单的初始化部分：首先需要维护一个数组，记录哪些端口已被绑定及其使用状态；此外还需要一个 `queue` 存放收到的包，并记录已收到的数量。
...
这里还新加了一个 `udp_pkt` 结构，是参照原本的 udp 结构改写，额外增加了两个字段：一是 `sip`（数据来源 IP），二是 `payload`（内容数据，即 UDP 的数据部分，不包括 eth/ip/udp 头）。明确知道 payload 大小有系统设定上限，直接用一个数组来存数据；后续会用 `kalloc` 新分配一块内存，然后数据拷贝进去，再释放原来的 buffer。

直接把 binding 相关的数组添加在 `net.c` 中，然后 `sys_bind` 完成绑定。
...
接下来实现 `ip_rx`，整体逻辑为：先用结构体将包中的数据解析出来，然后判断是否为 UDP 类型的请求（只处理 UDP 类型的请求），再从数组中查找匹配绑定的端口，然后在该端口对应的环形队列中找到可存放数据包的位置，写入相关字段，并使用 `memmove` 迁移数据；最后更新 `head` 和 `count` 状态，确保 binding 中的状态更新后。调用 `wakeup` 唤醒等待进程，然后用户那边的sys_recv就会开始工作。（关于这里 `wakeup` 对应的唤醒对象，会在接下来的 `sys_recv` 中提到。）
...
最后一步是实现 `sys_recv`。它需要拷贝 src IP、`sport`、payload（最多 `maxlen` 字节），并返回拷贝的字节数；若没有数据包则进入等待。下面是这个方法的传参和返回：
...
直接来看函数实现结果。需要注意的是，`sys_recv` 只需收到**一个** UDP 包就返回，因此除了用于等待的 `while` 之外，不再需要其他循环。
...
用 `argint` 和 `argaddr` 获取用户调用传入的参数，随后扫描整个 binding 数组，找到对应的端口。如果尚未收到数据，则调用 `sleep(&bindings[i], &netlock)` 挂起等待，在有数据到来时就会被唤醒——这正是前文 `ip_rx` 中调用 `wakeup` 唤醒的目标。之后依旧通过 `head` 计算索引，从环形队列中取出 payload，更新状态后，即可通过 `copyout` 将数据整体拷贝到用户进程中。
